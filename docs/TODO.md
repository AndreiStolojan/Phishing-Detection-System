...
- [x] **Faza 5 — Verdict UI (heart)**: VerdictBanner hero (icon ring 40px, left accent, glow one-shot pe loud, AnimatePresence keyed pe riskBucket); ReviewActions cu Check animat + Sonner toast (inclusiv side-effect Gmail Spam) + optimistic flip + rollback; ScanDetails narativ (AI explanation panel primar, fallback Ollama calm pe risk-review, Evidence colapsabil cu score bars animate din 0); culoare phishing = rose (quarantine) / brick (confirmed). Detail page nu mai face full-reload la review. RiskBadge lăsat ca atare (deja consistent; tooltip amânat ca să evit provider global). **Cele 3 teste ReviewActions reparate → 18/18 trec.** Build curat 2026-06-06.
- [x] **Faza 6 — Reading view + privacy gate**: reguli `.email-body` reale (`@layer components`, `:where()` 0-specificity); privacy gate în `sanitizeEmailHtml` (returnează `{html, blockedImages}`, blochează img remote + background-image + tracking pixels pe buckets riscante) + banner "Load images" în EmailBody; keyboard nav ←/k, →/j, Esc (guard la input); header cu monogram + "n of total"; surface de citire fără cutia dublă-border; ScanDetails inline (fără dublu-collapse) cu AI explanation mereu vizibil; re-scan cu toast. 20/20 teste trec (2 teste sanitize noi). Build curat 2026-06-06. (reading width 68ch sărit — ar strica layout tabele email; slide direcțional sărit — crossfade e suficient.)
- [x] **Faza 7 — Dashboard posture**: PostureHero band full-width ("You're protected" verde / "N need attention" rose + CTA Review now); RiskDonut mărit (208px) cu center = safe %, legend hover-linked + procente; StatCard count-up de la valoarea anterioară (nu de la 0 la fiecare sync) + reduced-motion guard, linkate la filtre cu hover lift + ArrowUpRight; attention list cu EmailRow `compact`; DashboardSkeleton mirror layout nou; naming SecureInbox în connect prompt. Build curat 2026-06-06.
