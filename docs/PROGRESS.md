...
- Config necesar: `EMAIL_FROM` și `EMAIL_PASSWORD` în `.env.development.local` sau în fișierul de env al mediului curent
...
- Migrare locală: dacă există duplicate vechi în `scans`, trebuie rulat `npm run cleanup:duplicate-scans` înainte ca MongoDB să poată construi indexul unic
...
- scriptul `npm run cleanup:duplicate-scans` curăță duplicatele vechi înainte de construirea indexului unic în development;
...
- rulează checklist-ul din `docs/MANUAL_TESTS.md` în ordinea recomandată;
- validează flow-ul cap-coadă `register -> login -> connect Gmail -> sync -> scan automat -> list emails -> mark-phishing -> verificare Gmail Spam`;
- test manual `PATCH /api/v1/users/me/ai-settings` cu `aiEnabled: 0`, apoi scanare și verificare `aiExplanationMeta.fallbackReason: ai_disabled`;
- test manual `PATCH /api/v1/users/me/ai-settings` cu `aiEnabled: 1`, apoi rescanare și verificare `aiExplanationMeta.status: generated` când Ollama răspunde valid;
- test manual fallback cu Ollama oprit sau invalid;
- test manual pentru `GET /api/v1/emails?verdict=safe`;
- test manual pentru `GET /api/v1/emails?verdict=likely_phishing`;
- test manual pentru `GET /api/v1/emails?verdict=phishing`;
- test manual pentru `GET /api/v1/emails?riskBucket=needs_review`;
- test manual pentru `GET /api/v1/emails?riskBucket=quarantine`;
- test manual pentru `GET /api/v1/emails?riskBucket=confirmed_phishing`.
