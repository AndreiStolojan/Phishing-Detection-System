...
- **Filtru global de interval:** context nou `TimeRangeContext` (+ `lib/timeRange.js` pur, testat) cu presets Last day (24h rulante) / Yesterday / Last 30 days (default) / Last 90 days / Last month (luna calendaristică precedentă). Limitele se calculează în timezone-ul browserului și pleacă spre API ca timestamps absolute `?from=&to=` — backend-ul doar compară instante, fără logică de timezone. Picker-ul (dropdown) trăiește pe dashboard, în locul vechiului badge static „Last 30 days"; starea e doar în memorie (refresh ⇒ default).
- **Backend `from`/`to`:** helper comun `backend/src/common/utils/date-range.js` (`INVALID_DATE_RANGE`, ambele capete obligatorii, `from < to`) folosit de `GET /emails` (filtru nou pe `receivedAt`), `/emails/stats`, `/emails/trend` (zero-fill pe zilele din interval), `/emails/top-risky-senders` și de rapoarte. La rapoarte `from`/`to` au prioritate peste `month` (care rămâne valid pentru digest/compatibilitate); modul range ancorează pe `receivedAt` (ca inbox-ul), modul month rămâne pe `createdAt` neschimbat. `label` opțional (doar pentru afișare, max 60 caractere, escaped) dă titlul emailului de raport; fallback: datele formatate Europe/Bucharest.
- **Totul ascultă de filtru:** dashboard (stats, trend, top senders, needs-attention, top rules), inbox (listă + chip counts, deci mereu consistente între ele) cu pastilă read-only a intervalului activ (link spre dashboard), `POST /reports/monthly-summary/send` primește același interval. Verificat în browser: switch pe „Yesterday" ⇒ toate request-urile pleacă cu exact același `from/to` (2026-06-09T21:00Z→2026-06-10T21:00Z, adică ziua calendaristică locală), dashboard + inbox arată aceleași 2 emailuri.
- **Reports desființat:** „Email me this report" (buton lângă picker, toast + stare „Emailed" resetată la schimbarea intervalului) și „Most common warning signs" (TopRulesChart mutat în `components/dashboard/` + legenda „What these mean"; `RULE_DESCRIPTIONS`/`getRuleDescription` centralizate în `lib/risk.js`) sunt acum pe dashboard. Șterse: `ReportsPage`, ruta `/reports`, intrarea din nav (nu există bottom nav — drawer-ul mobil folosește același array), `DetectionFunnel`, `RiskBreakdownBar`, `ReportsSkeleton`; `reportsApi` vorbește acum în `{from,to,label}`.
- **Layout:** scos `max-w-6xl` din `AppShell` (desktop full-width; `px-5` = exact 20px gutters pe mobil). Fix de overflow pe mobil: rândurile `grid gap-4 lg:grid-cols-5` aveau coloana implicită `auto` (track-ul se dimensiona la max-content și împingea pagina la ~547px); acum `grid-cols-1` explicit ⇒ toate paginile au `scrollWidth === 380` la viewport 380px (verificat pe dashboard/inbox/email detail/sender-lists/settings).
- **Stil carduri:** `rounded-xl` → `rounded-sm` în `ui/card.jsx` (toate cardurile); donut-ul de risc nu mai reacționează la hover (scos dim-ul pe felii + highlight-ul din legendă).
- **Contrast WCAG 2.1 AA:** audit cu script (luminanță WCAG + reproducere exactă `color-mix(in oklab …)` pentru fundalurile `-soft`). Singura culoare picată: `--color-risk-phishing` #a855f7 — 4.21:1 ca text pe fundalul soft al badge-ului „Confirmed phishing" (exact ce semnalase reviewerul). Corectat la **#b873f9** (doar lightness, rămâne violet): 5.96:1 pe card, 5.29:1 pe soft; toate cele 5 culori de risc trec acum 4.5:1 (text) și 3:1 (UI/donut) pe background, card și soft. Schimbarea e doar în `index.css` (token central).
- **Fix mediu de test (pre-existent):** jsdom 27 a eliminat `localStorage`/`sessionStorage`, picând 4 teste vechi (`apiClient`, `authContext`) și la HEAD; stub minimal de Storage în `vitest.setup.js`.
- **Verificat:** backend lint + 52/52 (+1 skip; +5 teste `date-range`), frontend build + 36/36 (+8 teste `timeRange`); verificare live cu cont demo temporar (creat prin API, emailuri din seeder, șters la final prin `DELETE /users/me`).

Următorul pas recomandat: demo-ul cap-coadă după `docs/PRESENTATION_SCRIPT.md` (de notat: scriptul menționează pagina Reports — raportul se demonstrează acum de pe dashboard), apoi draftul lucrării.

## Notă sesiune 2026-06-10 (4) - Script de prezentare + seeder demo

- `docs/PRESENTATION_SCRIPT.md`: script complet de prezentare pentru coordonator (10–12 min, în română) — problema/ideea, turul aplicației, 4 scenarii de detecție (phishing de credențiale, malware prin atașament, brand verificat ≠ fals pozitiv, email curat), demo-ul regulilor trust/block cu rescan live, review manual, rapoarte, închidere pe arhitectură + Q&A anticipat cu răspunsuri.
- `backend/scripts/seed-demo-inbox.js`: inserează 6 emailuri demo craftate (acoperă toate scenariile) în contul dat ca argument; `--clean` le șterge exact pe ele (prefix `demo-` pe providerMessageId). Documentele sunt doar în DB local — nu ating Gmailul real. Verificat: rulează din rădăcina repo-ului, error path corect pentru utilizator inexistent.

## Notă sesiune 2026-06-10 (3) - Pagină dedicată liste, conflicte cross-kind, dashboard produs, trust score aliniat

Feedback Andrei după prima versiune a listelor: (1) e ciudat că poți bloca un sender și apoi să marchezi domeniul lui ca de încredere — blocarea „dispare"; (2) listele merită pagină proprie, nu secțiune în Settings; (3) graficele din dashboard pot fi mai aproape de un produs real (+ schimbare culori); (4) trust score-ul din Reports diferă de cel din Dashboard.

- **Conflicte cross-kind interzise:** `findOppositeCoverageConflict` în `sender-list.service.js` — un sender rule și un domain rule care îl acoperă nu mai pot avea tipuri opuse (409 `LIST_CONFLICT` cu mesaj explicit în ambele direcții, suffix-aware). Suprapunerea pe aceeași direcție rămâne permisă (redundantă, inofensivă). Meniul „Trust / Block" de pe email reflectă regula: senderul acoperit de un domain rule afișează „Already trusted/blocked through the domain rule below" în loc de opțiuni contradictorii; pentru domeniu se oferă doar direcția compatibilă cu sender rule-ul existent.
- **Pagină dedicată `/sender-lists` („Trusted & Blocked" în sidebar):** `SenderListsPage.jsx` — carduri sumar (reguli trusted/blocked, emailuri acoperite), listă cu căutare + filtre All/Trusted/Blocked, badge-uri, număr de emailuri pe regulă (`GET /sender-lists?withMatchCounts=1`, count în `email.service` pe `from`/`senderDomain`), formular de adăugare, card „How rules work". Secțiunea din Settings înlocuită cu un pointer către pagină; componenta veche ștearsă.
- **Dashboard:** card nou „Who is targeting you" — top domenii din spatele emailurilor riscante pe 30 zile (endpoint nou `GET /emails/top-risky-senders`, agregare effective-bucket pe `senderDomain`), bare stivuite pe severitate, click → inbox filtrat pe domeniu. Paletă risc reîmprospătată pentru fundal întunecat (`#3ddc97`/`#fbbf24`/`#fb637e`/`#a855f7` — centralizat în `index.css`, se propagă în toate graficele + badge-uri), gradient mai bogat pe trend. Fix gramatical „1 email needs your review".
- **Trust score aliniat:** cauza diferenței — Reports folosea verdictul brut de scan (`counts.safe`), Dashboard folosea verdictul efectiv (review-ul utilizatorului suprascrie scanul). Agregarea lunară calculează acum split-ul efectiv (`effectiveSafe/Suspicious/LikelyPhishing/MarkedPhishing` — fiecare email scanat exact o dată, fără dublă numărare), iar Safe rate + Risk breakdown din Reports îl folosesc, cu etichetă de lună explicită. Verificat live: ambele ecrane arată identic 20% → 40% după un mark-safe.
- **Verificare live completă** (cont de test prin UI, șters la final prin Delete account — cascade OK): conflict cross-kind 409 în ambele direcții, pagina nouă cu counts reale, dashboard cu date, meniu „covered by domain rule". Backend 47/47 (+1 skip), frontend 28/28, lint + build curate.

## Notă sesiune 2026-06-10 (2) - Allowlist/Blocklist per utilizator (sender + domeniu)

Context: lista de încredere exista doar hardcodat (branduri verificate în `brand-domains.config.js`). Utilizatorul nu putea decide singur pe cine să blocheze sau în cine să aibă încredere.

- **Model nou `SenderListEntry`** (`models/sender-list.model.js`): `userId`, `listType` (`allow`/`block`), `kind` (`sender`/`domain`), `value` normalizat (lowercase, fără `www.`). Index unic `(userId, kind, value)` ⇒ **exclusivitate mutuală garantată la nivel de DB**: un criteriu are o singură intrare, deci nu poate fi simultan și blocat și de încredere. Serviciul răspunde 409 `LIST_CONFLICT` cu mesaj prietenos la încercarea de a adăuga pe lista opusă.
- **Potrivire** (`services/sender-list.service.js`): intrarea exactă de sender bate intrarea de domeniu (criteriul mai specific câștigă, ex. `boss@firma.com` trusted deși `firma.com` e blocat); domeniile se potrivesc pe sufix ca la verificarea de brand (`mail.x.com` ⊂ `x.com`, dar NU `evil-x.com`).
- **Ponderare hibridă** (decizie Andrei, în DECISIONS.md): blocklist = regulă dură `user_blocklist_match` cu exact `RISK_THRESHOLDS.likelyPhishing` (60) puncte — verdictul `likely_phishing` e garantat prin construcție, semnalele reale se adaugă peste; allowlist = strat de modificatori `USER_ALLOWLIST_MODIFIERS` (mai puternic decât brandul verificat: contextualele ×0, social-eng/arhive ×0.5), dar semnalele critice (cerere de date sensibile, atașamente periculoase, IP-link, credențiale în URL, punycode) rămân la pondere plină — acoperă scenariul „expeditor de încredere compromis". Ponderea de blocklist stă în afara `RULE_WEIGHTS` ca invariantele euristice să rămână valabile. Combinare straturi prin multiplicator minim (`applyScoreContextModifiers`). Engine `rules-ai-v6`→`rules-ai-v7`.
- **API nou:** `GET/POST /api/v1/sender-lists`, `DELETE /api/v1/sender-lists/:id` (Joi: email valid pentru sender, domeniu valid pentru domain). Scanarea persistă `senderListMatch` pe `Scan` și îl expune în API.
- **Frontend:** secțiune „Trusted & blocked senders" în Settings (adăugare cu select Trust/Block + Sender/Domain, listă cu badge-uri colorate, ștergere); pe pagina emailului buton „Trust / Block" (dropdown cu acțiuni pe sender și pe domeniu, stare curentă reflectată), chip „On your trusted/blocked list", banner în panoul de scanare, toast cu acțiune „Scan again" (listele se aplică doar la scanări viitoare — decizie Andrei).
- **Teste:** nou `tests/unit/sender-list.test.js` (12 teste: normalizare, precedență, sufix, lookalike, podea blocklist, mut contextual + critice intacte la allowlist, multiplicator minim). Backend 47/47 (+1 skip DB), frontend 28/28, lint + build curate.
- **Verificare live:** smoke test complet pe API (409 conflict, idempotent, normalizare, 404 la ștergere dublă) + scan real: email blocat 70/100 `likely_phishing`, același email cu sender trusted 3/100 `safe`, după ștergerea intrării 67/100 — ciclul complet funcționează. Verificat și în UI (preview browser): Settings add/remove + conflict toast, Trust/Block pe email, rescan 38→0 (trust) și →100 (block).
